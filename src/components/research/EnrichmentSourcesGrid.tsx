      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {SOURCE_CATEGORIES.map((category) => (
          ...
          >
            ...
            <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-navySoft mt-3 mb-2">
              {category.heading}
            </p>
            ...
            <ul className="space-y-1.5">
              {category.sources.map((source) => (
                <li key={source} className="flex items-start gap-2 text-sm text-ink">
                  <span className="text-navySoft font-mono mt-0.5 leading-none shrink-0" aria-hidden="true">→</span>
                  <span>{source}</span>
                </li>
              ))}
            </ul>
