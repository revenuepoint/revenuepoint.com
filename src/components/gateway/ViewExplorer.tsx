    ...
    <div className="border border-border rounded-md overflow-hidden bg-white">
      ...
      <table className="w-full text-sm">
        <thead className="bg-offWhite border-b border-border">
          <tr>
            <th className="text-left py-2 px-4 text-[11px] uppercase tracking-widest font-semibold text-mutedText">{tenant.recordIdLabel} #</th>
            <th className="text-left py-2 px-4 text-[11px] uppercase tracking-widest font-semibold text-mutedText">Subject</th>
            <th className="text-left py-2 px-4 text-[11px] uppercase tracking-widest font-semibold text-mutedText">Status</th>
            <th className="text-right py-2 px-4 text-[11px] uppercase tracking-widest font-semibold text-mutedText">Amount</th>
            <th className="text-right py-2 px-4 text-[11px] uppercase tracking-widest font-semibold text-mutedText">Updated</th>
          </tr>
        </thead>
        <tbody>
          {tenant.records.map((r, i) => (
            <tr key={r.id} className={i % 2 === 0 ? 'bg-white' : 'bg-lightGray'}>
              <td className="py-2.5 px-4 font-mono text-xs text-navy">{r.id}</td>
              <td className="py-2.5 px-4 text-bodyText">{r.title}</td>
              ...
              <td className="py-2.5 px-4 text-right text-xs text-mutedText">{r.updated}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function DetailView() {
  ...
  return (
    <div className="border border-border rounded-md overflow-hidden bg-white">
      <div
        className="px-4 py-3 border-b border-border flex items-start justify-between gap-4"
        style={{ background: tenant.primaryColorTint }}
      >
        <div>
          <p
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 p-5">
        {tenant.detailFields.map((f) => (
          <div key={f.label} className="flex flex-col gap-0.5">
            <span className="text-[10px] uppercase tracking-widest text-mutedText">{f.label}</span>
      ...
      <div className="border-t border-border px-5 py-3 flex items-center gap-4">
        {['Comments', 'Attachments', 'Activity'].map((tab, i) => (
          <span
            key={tab}
            className={`text-xs ${i === 0 ? 'font-semibold text-navy border-b-2 pb-1.5' : 'text-mutedText pb-1.5'}`}
...
function FormView() {
  ...
  return (
    <div className="border border-border rounded-md overflow-hidden bg-white">
      ...
      <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
        {tenant.formFields.map((f) => (
          <div key={f.label} className={`flex flex-col gap-1 ${f.type === 'textarea' ? 'md:col-span-2' : ''}`}>
            <label className="text-[11px] uppercase tracking-widest text-mutedText font-semibold">{f.label}</label>
            {f.type === 'select' ? (
              <select
                className="border border-border rounded-sm px-3 py-2 text-sm text-navy bg-white focus:outline-none"
                disabled
                defaultValue=""
              >
              ...
              <textarea
                className="border border-border rounded-sm px-3 py-2 text-sm text-navy bg-white focus:outline-none resize-none"
                rows={3}
      ...
      <div className="border-t border-border px-5 py-3 flex items-center justify-end gap-2 bg-offWhite/40">
        <span className="text-xs px-3 py-1.5 rounded-sm border border-border bg-white text-mutedText font-semibold">
          Cancel
        </span>
        <span
          className="text-xs px-3 py-1.5 rounded-sm font-semibold text-white"
    ...
    <div className="border border-border rounded-md overflow-hidden bg-white">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 p-5">
        {tenant.dashboardKpis.map((k) => (
          <div key={k.label} className="border border-border rounded-md p-3 bg-white">
            <p className="text-[10px] uppercase tracking-widest text-mutedText font-semibold">{k.label}</p>
      ...
      <div className="px-5 pb-5">
        <p className="text-[10px] uppercase tracking-widest text-mutedText font-semibold mb-2">
          Recent {tenant.recordObjectLabel.toLowerCase()}
        </p>
        <div className="flex flex-col gap-1.5">
          {tenant.records.slice(0, 3).map((r) => (
            <div key={r.id} className="flex items-center gap-3 border border-border rounded-sm px-3 py-2 bg-white">
              <span className="text-[11px] font-mono text-mutedText shrink-0 w-20 truncate">{r.id}</span>
...
function TabsView() {
  ...
  return (
    <div className="border border-border rounded-md overflow-hidden bg-white">
      <div
        className="px-4 pt-3 border-b border-border flex items-end gap-1"
        style={{ background: tenant.primaryColorTint }}
      >
        {tabs.map((t, i) => (
          <span
            ...
            className={`text-xs px-3 py-2 ${
              i === 0 ? 'bg-white text-navy font-semibold border border-border border-b-0 rounded-t-sm' : 'text-mutedText'
            }`}
          >
            {t.label}
          </span>
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-4 lg:gap-6">
        <div className="border border-border rounded-md bg-white p-5">
          ...
          <p className="text-sm text-bodyText leading-relaxed mb-4">{view.description}</p>
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1.5">
            {view.capabilities.map((c) => (
              <li key={c} className="flex items-start gap-2 text-xs text-bodyText">
        ...
        <div className="border border-border rounded-md bg-navy text-white p-5">
...
{view.sampleConfig}
      ...
      {/* Live preview */}
      <div className="rounded-lg border border-border bg-offWhite p-4 lg:p-6 min-h-[420px]">
