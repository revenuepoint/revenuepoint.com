      ...
      <ol className="mt-3 space-y-2">
        {tasks.map((t) => (
          <li key={t.time} className="flex items-start gap-3">
            <span className="shrink-0 mt-0.5 inline-flex items-center justify-center min-w-[40px] px-1.5 py-0.5 rounded-sm bg-crimsonLight text-crimson text-[10px] font-bold tracking-wide">
              {t.time}
            </span>
            <div className="text-xs">
              <div className="font-semibold text-navy leading-snug">{t.title}</div>
              <div className="mt-0.5 text-[11px] text-bodyText leading-relaxed">{t.detail}</div>
            </div>
          </li>
        ))}
      </ol>
