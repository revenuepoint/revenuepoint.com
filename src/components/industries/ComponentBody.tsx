  ...
  if (body.kind === 'kpi-tiles') {
    return (
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {body.tiles.map((t) => (
          <div key={t.label} className="border border-border rounded-md bg-white p-3">
            <div className="text-[10px] uppercase tracking-widest text-mutedText">{t.label}</div>
          ...
          <thead className="bg-offWhite">
            <tr>
              {body.headers.map((h) => (
                <th
                  key={h}
                  className="text-left px-3 py-2 text-[10px] uppercase tracking-widest text-mutedText font-semibold"
                >
                  {h}
                </th>
              ))}
            ...
            {body.rows.map((r, i) => (
              <tr key={i} className="border-t border-border">
                {r.cells.map((c, j) => (
                  <td
                    key={j}
                    className={`px-3 py-2 ${j === 0 ? 'font-medium text-navy' : 'text-bodyText'} ${
        ...
        {body.entries.map((e, i) => (
          <li key={i} className="flex items-start gap-3 text-xs">
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="text-mutedText">{e.sub}</div>
            </div>
          </li>
        ))}
      </ol>
  ...
  if (body.kind === 'field-list') {
    return (
      <dl className="grid grid-cols-2 gap-x-4 gap-y-2.5 text-xs">
        {body.fields.map((f) => (
          <div key={f.label}>
            <dt className="text-[10px] uppercase tracking-widest text-mutedText">{f.label}</dt>
