  ...
  return (
    <div>
      ...
      <table className="mt-3 w-full text-xs">
        <tbody>
          {rows.map((r) => (
            <tr key={r.score} className="border-b border-border last:border-b-0">
              ...
              <td className="py-2 text-bodyText">{r.desc}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div className="mt-4 flex gap-3 text-[10px] uppercase tracking-widest">
        <span className="inline-flex items-center gap-1.5 text-red">
          <span className="w-2 h-2 rounded-full bg-red" /> Critical
        </span>
        ...
        <span className="inline-flex items-center gap-1.5 text-green">
          <span className="w-2 h-2 rounded-full bg-green" /> Advisory
        </span>
      </div>
    </div>
  );
