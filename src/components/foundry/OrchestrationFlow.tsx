    ...
    <div className="mt-8 flex flex-wrap items-stretch justify-center gap-2">
      {STEPS.map((s, i) => (
        <div key={s.label} className="flex items-stretch gap-2">
          <div className="rounded-md border border-border bg-white px-3 py-2 min-w-[140px]">
            ...
            <p className="text-[11px] text-bodyText mt-0.5">{s.copy}</p>
          </div>
          {i < STEPS.length - 1 && (
            <div className="flex items-center text-mutedText text-lg font-light select-none" aria-hidden>
              →
            </div>
          )}
        </div>
